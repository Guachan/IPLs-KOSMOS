...
diat_lipids = importance(forest.T) %>% as.data.frame() %>% 
        rename(RMSE = "%IncMSE") %>% 
        arrange(desc(RMSE)) %>% 
        slice(1:20)
        
diat_names = rownames(diat_lipids)

    

revised_data <- data[diat_names] %>% 
        cbind(Y)
...
#Plot error VS trees
plot(forest.T, main="Random Forest for diat (mg/L)")

#Plot observed VS predicted
png(filename = "diat_R2.png", width = 500, height = 500, units = "px")
par(mar=c(5,5,2,2))
plot(revised_data$Y,  forest.T.Pred, xlim = forest.myRange, ylim = forest.myRange, xlab="Observed diat Concentration (mg/L)", ylab="Modeled diat Concentration (mg/L)", cex.lab =1.75, cex.axis = 1.5, cex = 2)
...
chloro_lipids = importance(forest.T) %>% as.data.frame() %>% 
    rename(RMSE = "%IncMSE") %>% 
    filter(RMSE > 2.49) %>% 
    arrange(desc(RMSE))

chloro_names = rownames(chloro_lipids)



revised_data <- data[chloro_names] %>% 
    cbind(Y)
...
#Plot error VS trees
plot(forest.T, main="Random Forest for chloro (mg/L)")
...
chloro_lm <- lm(Y ~., data = revised_data)

pred_chloro <- predict(chloro_lm, newdata = data, se.fit = TRUE) %>% as_tibble() %>% 
        bind_cols(data)
formula <-  y ~ x
ggplot(data = pred_chloro, aes(fit, Y))+
        geom_point()+
