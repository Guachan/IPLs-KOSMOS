...
#Plot error VS trees
plot(forest.T, main="Random Forest for crypto (mg/L)")
...
crypto_lipids = importance(forest.T) %>% as.data.frame() %>% 
        rename(RMSE = "%IncMSE") %>% 
        arrange(desc(RMSE)) %>% 
        slice(1:20)
        
crypto_names = rownames(crypto_lipids)

    

revised_data <- data[crypto_names] %>% 
        cbind(Y)
...
#Plot observed VS predicted
png(filename = "Crypto_R2.png", width = 500, height = 500, units = "px")
par(mar=c(5,5,2,2))
plot(revised_data$Y,  forest.T.Pred, xlim = forest.myRange, ylim = forest.myRange, xlab="Observed crypto Concentration (mg/L)", ylab="Modeled crypto Concentration (mg/L)", cex.lab =1.75, cex.axis = 1.5, cex = 2)
...
crypto_lm <- lm(Y ~., data = revised_data)

pred_crypto <- predict(crypto_lm, newdata = data, se.fit = TRUE) %>% as_tibble() %>% 
        bind_cols(data)
formula <-  y ~ x
ggplot(data = pred_crypto, aes(fit, Y))+
        geom_point()+
