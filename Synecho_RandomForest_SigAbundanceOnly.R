...
synecho_lipids = importance(forest.T) %>% as.data.frame() %>% 
        rename(RMSE = "%IncMSE") %>% 
        arrange(desc(RMSE)) %>% 
        slice(1:20)
        
synecho_names = rownames(synecho_lipids)

    

revised_data <- data[synecho_names] %>% 
        cbind(Y)
...
#Plot error VS trees
plot(forest.T, main="Random Forest for synecho (mg/L)")

#Plot observed VS predicted
png(filename = "synecho_R2.png", width = 500, height = 500, units = "px")
par(mar=c(5,5,2,2))
plot(revised_data$Y,  forest.T.Pred, xlim = forest.myRange, ylim = forest.myRange, xlab="Observed synecho Concentration (mg/L)", ylab="Modeled synecho Concentration (mg/L)", cex.lab =1.75, cex.axis = 1.5, cex = 2)
...
synecho_lm <- lm(Y ~., data = revised_data)

pred_synecho <- predict(synecho_lm, newdata = data, se.fit = TRUE) %>% as_tibble() %>% 
        bind_cols(data)
formula <-  y ~ x
ggplot(data = pred_synecho, aes(fit, Y))+
        geom_point()+
